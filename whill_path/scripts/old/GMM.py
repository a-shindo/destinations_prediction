...
"""
https://aidiary.hatenablog.com/entry/20150322/1427026731
"""
...
def scale(X):
    ...
    # 属性ごとデータを標準化
    for i in range(col):
        X.iloc[:,i] = (X.iloc[:,i] - mu[i]) / sigma[i]

    return X

# データをロード
data = pd.read_csv('/home/ytpc2017d/catkin_ws/src/whill_path/scripts//total_dining_hall_1.csv')
X_train = scale(data)
print("X_train = scale(data)",X_train )
N = len(X_train)
...
# 訓練データを描画
plt.plot(X_train.iloc[:, 0], X_train.iloc[:, 1], 'gx')
